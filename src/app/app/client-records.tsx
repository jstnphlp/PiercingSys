  ...
  return <>
    ...
    {!error && !isLoading && visibleCustomers.length ? <section className={tablePanel}><table><thead><tr><th>Client</th><th>Contact</th><th>Appointments</th><th>Last activity</th></tr></thead><tbody>{visibleCustomers.map((customer) => {
      return <tr key={customer.id} className="cursor-pointer hover:bg-[#f7dfb3] focus:bg-[#f7dfb3] focus:outline-2 focus:-outline-offset-2 focus:outline-[#d66335]" tabIndex={0} onClick={() => setSelected(customer)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(customer); } }}>
        <td><strong>{customer.name}</strong></td><td>{customer.email || customer.phone ? <><span>{customer.email}</span><small>{customer.phone}</small></> : <span className="text-studio-muted">No contact details</span>}</td><td>{customer.appointmentCount ?? 0}</td><td>{formatDate(customer.lastActivityAt ?? customer.createdAt)}</td>
      </tr>;
...
function ClientDialog({ customer, onClose }: { customer: CustomerRecord; onClose: () => void }) {
  ...
  return <div className={operationBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className={operationDialog} role="dialog" aria-modal="true" aria-labelledby="client-dialog-title" tabIndex={-1} ref={ref}>
    ...
    <div className="flex flex-wrap gap-2.5 border-b border-dashed border-[#c88f6e] px-[21px] py-4 [&_a]:flex [&_a]:min-h-[38px] [&_a]:items-center [&_a]:gap-[7px] [&_a]:rounded-[10px] [&_a]:border-[1.5px] [&_a]:border-hippy-ink [&_a]:bg-[#f8d7a5] [&_a]:px-3 [&_a]:py-2.5 [&_a]:text-[11px] [&_a]:font-bold [&_a]:text-[#50362e] [&_a]:shadow-[2px_2px_0_#3b2923] [&_a:hover]:bg-[#f2c590] [&_svg]:w-[15px]">{customer.email || customer.phone ? <>{customer.email && <a href={`mailto:${customer.email}`}><Mail/> {customer.email}</a>}{customer.phone && <a href={`tel:${customer.phone}`}><Phone/> {customer.phone}</a>}</> : <p className="m-0 text-[10px] font-bold text-studio-muted">Walk-in client · No contact details</p>}</div>
