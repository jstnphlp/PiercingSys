...
export function SalesView({ initialSales, services }: { initialSales: SaleRecord[]; services: Service[] }) {
  ...
  return <div className="feature-view">
    ...
    <div className="list-toolbar"><label className="field">Search sales<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Reference or client" /></label>{isValidating && !isLoading && <span className="status-note">Refreshing…</span>}</div>
