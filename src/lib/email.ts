...
export async function deliverBookingEmail(deliveryId: string) {
  ...
  if (!admin) return;
  const { data: delivery } = await admin.from("notification_deliveries").select("id,kind,recipient,status,attempts,bookings(reference,starts_at,customers(first_name),booking_services(position,name))").eq("id", deliveryId).single();
