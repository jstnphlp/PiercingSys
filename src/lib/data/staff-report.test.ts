...
describe("report data loading", () => {
  it("passes the same inclusive Manila UTC boundaries to the reporting RPC", async () => {
    const rpc = vi.fn().mockResolvedValue({
      data: { revenue_cents: 125000, completed_sales: 1, booking_statuses: { completed: 2 }, methods: { cash: 125000 } },
      error: null,
    });
    ...
    expect(data).toMatchObject({ completedRevenueCents: 125000, completedSaleCount: 1 });
  });
});
